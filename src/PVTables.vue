<template>
  <div class="card">
    <Toolbar class="p-mb-4">
      <template #start>
        <Button v-for="action in actions.filter(x=>x.head)" :icon="action.icon" 
          :label="action.label"
          :class="action.class" 
      ...
      <template #end>
        <Button icon="pi pi-refresh" 
          ...
          @click="refresh" 
          />
        <Button type="button" icon="pi pi-filter-slash" @click="clearFilter()" />
      </template>
    </Toolbar>
    <DataTable :value="lineItems" lazy paginator :first="first" :rows="10" :rowsPerPageOptions="[10,60,30,10]" ref="dt" dataKey="id"
      :totalRecords="totalRecords" :loading="loading"
      @page="onPage($event)" @sort="onSort($event)" sortMode="multiple"
      ...
      <template v-for="col of columns.filter(x=>x.modal_only!=true)" :key="col.field">
        <Column v-if="col.field=='id'" field="id" header="id" style="padding: 1rem 10px 1rem 10px;" sortable>
          <template #body="{ data, field }">
              {{ data[field]}} 
          </template>
        </Column>
        <Column v-else :field="col.field" :header="col.label" style="min-width: 12rem" sortable>
          ...
          <template v-else-if="col.type=='decimal'" #editor="{ data, field }">
            <InputNumber v-model="data[field]":minFractionDigits="col.FractionDigits" :maxFractionDigits="col.FractionDigits" />
          </template>
          ...
          <template #filter="{ filterModel }">
            <InputText v-model="filterModel.value" type="text" class="p-column-filter" :placeholder="filterPlaceholder(col)" />
          </template>
        </Column>
      </template>
      <Column v-if="actions_row" :exportable="false" style="white-space: nowrap;">
        <template #body="slotProps">
            <Button v-for="action in actions.filter(x=>x.row)" :icon="action.icon" 
            :class="action.class" 
      ...
      <template #expansion="slotProps">
        <div class="p-3">
          <PVTables :table="subtables[slotProps.data.id]"
      ...
      <template v-for="col of columns.filter(x=>x.table_only!=true)">
        <div class="p-field">
            ...
            <template v-else-if="col.type=='decimal'">
              <InputNumber :id="col.field" v-model="lineItem[col.field]" 
              :minFractionDigits="col.FractionDigits" :maxFractionDigits="col.FractionDigits"/>
            </template>
...
<script setup>
    ...
    for(let field in fields){
      if(props.filters.hasOwnProperty(field)){
        ...
        switch(fields[field].type){
          default:
            filters0[field] = { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] }
        }
  ...
  onMounted(async () => {
    ...
    try {
      const response = await axios.options('/api/' + props.table);
      // console.log(response);
      if(!response.data.success){
        mytoast.add({ severity: 'error', summary: 'Ошибка', detail: response.data.message, life: 3000 })
        return
      }
        ...
        for(let action in actions0){
          ...
          switch(action){
            case 'update':
              ...
              if(!tmp.hasOwnProperty('class')) tmp.class="p-button-rounded p-button-success"
              if(!tmp.hasOwnProperty('click')) tmp.click = (data) => editLineItem(data)
            break
            case 'delete':
              if(!tmp.hasOwnProperty('row')) tmp.row = true
              if(!tmp.hasOwnProperty('head')) tmp.head = true
              if(!tmp.hasOwnProperty('icon')) tmp.icon="pi pi-trash"
              if(!tmp.hasOwnProperty('class')) tmp.class="p-button-rounded p-button-danger"
            ...
            case 'create':
              ...
              if(!tmp.hasOwnProperty('class')) tmp.class="p-button-rounded p-button-success"
              if(!tmp.hasOwnProperty('head_click')) tmp.head_click = () => openNew()
            ...
            case 'subtables':
              ...
              for(let tmptable in actions0[action]){
                ...
                if(!tmpt.hasOwnProperty('class')) tmpt.class="p-button-rounded p-button-success"
  ...
  const loadLazyData = (event) => {
    ...
    axios.get('/api/' + props.table,{ params: params})
  ...
  const onCellEditComplete = async (event) => {
    ...
    try {
      const response = await axios.patch('/api/' + props.table, {id: data.id, [field]: newValue})
      if(!response.data.success){
        mytoast.add({ severity: 'error', summary: 'Ошибка', detail: response.data.message, life: 3000 })
        return
      }
  ...
  const saveLineItem = () => 
  {
    ...
    {
      axios.patch('/api/' + props.table, lineItem.value)
      .then(response => {
        if(!response.data.success){
          mytoast.add({ severity: 'error', summary: 'Ошибка', detail: response.data.message, life: 3000 })
          return
      ...
      axios.put('/api/' + props.table, lineItem.value)
      .then((response) =>{ 
        if(!response.data.success){
          mytoast.add({ severity: 'error', summary: 'Ошибка', detail: response.data.message, life: 3000 })
          return
  ...
  {
    axios.delete('/api/' + props.table + '?ids=' + lineItem.value.id)
  ...
  {
    if(selectedlineItems.value && selectedlineItems.value.length) deleteLineItemsDialog.value = true;
  };
  const deleteSelectedLineItems = () => 
  {
    ...
    axios.delete('/api/' + props.table + '?ids=' + ids.join(','))
      .then((response) => {
        if(!response.data.success){
          mytoast.add({ severity: 'error', summary: 'Ошибка', detail: response.data.message, life: 3000 })
          return
