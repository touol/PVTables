...
// Опции переключаемых таблиц (поля с table_by): сервер отдаёт их отдельными
// наборами, потому что id в разных справочниках совпадают и одной картой
// их не различить.
const acMapsByTable = computed(() => {
  const maps = {}
  for (const field in autocompleteSettings.value) {
    const byTable = autocompleteSettings.value[field]?.by_table
    if (!byTable) continue
    maps[field] = {}
    for (const table in byTable) {
      const rows = byTable[table]?.rows
    ...
    cell: ({ getValue, row }) => {
      ...
      switch (effectiveType) {
        ...
        case 'autocomplete': {
          ...
          // У поля с table_by строка может ссылаться на другую таблицу — тогда
          // и подпись берём из её набора опций, а не из общего списка колонки:
          // id в справочниках совпадают, и общий список показал бы чужую запись.
          const acTable = col.table_by ? resolveFieldTable(col, data) : col.table
          const lbl = acTable !== col.table
            ? (acMapsByTable.value[col.field]?.[acTable]?.get(String(value)) ?? '')
